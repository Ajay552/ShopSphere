...
**ShopSphere** is a conversational retail assistant that demonstrates LangChain tool-calling agents against a **SQLite** catalog (seeded from JSON under `data/`). It is built in three progressive modes: a basic ReAct-style agent, a “deep” agent with optional **Tavily** web search, and a **keyword-routed multi-agent** setup.
...
| Data | SQLite (`data/shopsphere.db`), seeded from `data/*.json` |
...
Seed the database from JSON (required on first setup, or after editing seed files):

```bash
python scripts/seed_db.py
```

Re-import everything from JSON (drops and recreates tables):

```bash
python scripts/seed_db.py --force
```

Optional: set `DATABASE_URL` for a different backend later (e.g. Postgres). Default is `sqlite:///data/shopsphere.db`.
...
├── data/
│   ├── products.json           # seed source (not read at runtime)
│   ├── orders.json
│   ├── coupons.json
│   ├── store_hours.json
│   └── shopsphere.db           # generated by seed script (gitignored)
├── scripts/
│   └── seed_db.py              # import JSON into SQLite
├── prompts/
...
├── tools/
│   ├── db.py                   # engine, DATABASE_URL, ensure_db_seeded()
│   ├── models.py               # SQLAlchemy ORM models
│   ├── repository.py           # get_products(), get_orders(), etc.
│   ├── seed.py                 # JSON → SQLite import logic
│   ├── utils.py                # load_prompt(), get_llm()
...
Agent tools read catalog data through `tools.repository` (SQLite). JSON files under `data/` are **seed sources only** — edit them, then run `python scripts/seed_db.py --force` to refresh the database. On first use, `ensure_db_seeded()` auto-imports if the DB is missing.

System prompts live in `prompts/` and load via `tools.utils.load_prompt()`.

**Note:** `cancel_order` and `process_return` still return confirmation messages without writing to the database (mock behavior).
...
- **Stale or missing data** — Run `python scripts/seed_db.py` (or `--force` after editing JSON seed files). Delete `data/shopsphere.db` and re-seed if the schema gets out of sync.
